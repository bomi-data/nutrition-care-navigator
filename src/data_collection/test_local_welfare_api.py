"""
한국사회보장정보원_지자체복지서비스 API 연결/구조 테스트.

공식 명세 (공공데이터포털 15108347 상세페이지에 내장된 Swagger 2.0 문서를 직접 파싱하여 확인, 2026-08-21):
  Base URL : https://apis.data.go.kr/B554287/LocalGovernmentWelfareInformations
  목록조회 : GET /LcgvWelfarelist
             필수 파라미터: serviceKey (pageNo/numOfRows/ctpvNm/sggNm/searchWrd 등은 모두 옵션)
  상세조회 : GET /LcgvWelfaredetailed
             필수 파라미터: serviceKey, servId
  응답형식 : application/xml (Swagger 'produces'에 application/xml만 명시됨. JSON 미지원)

주의: 중앙부처복지서비스 API와 응답 스키마가 다르다(예: 지원대상 필드명이 여기서는
sprtTrgtCn, 중앙부처는 tgtrDtlCn). 두 API를 같은 스키마로 취급하지 않는다.

이 스크립트는 목록 5건 + 그중 1건 상세조회만 호출한다. 전체 데이터 수집은 하지 않는다.
API 인증키는 .env의 LOCAL_WELFARE_API_KEY에서만 읽으며, 코드에 직접 작성하지 않는다.
"""
import os
import sys
...
def main():
    ...
    # 1) 목록조회 (5건, 지역/검색어 미지정 — 구조 확인 목적)
    list_params = {
        "serviceKey": service_key,
        "pageNo": "1",
        "numOfRows": str(SAMPLE_COUNT),
    }
    list_xml = call(LIST_ENDPOINT, list_params, "지자체복지서비스 목록조회")
    if list_xml is None:
        print("\n목록조회에 실패하여 상세조회로 진행하지 않습니다.")
        sys.exit(1)

    list_path = RAW_DIR / "local_welfare_sample.xml"
    ...
    for item in items:
        ...
        ctpv_nm = item.findtext("ctpvNm")
        sgg_nm = item.findtext("sggNm")
        print(f"  - servId={serv_id} servNm={serv_nm} ctpvNm={ctpv_nm} sggNm={sgg_nm}")
    ...
    detail_xml = call(
        DETAIL_ENDPOINT, detail_params, f"지자체복지서비스 상세조회 (servId={first_serv_id})"
    )
    if detail_xml is None:
        return

    detail_path = RAW_DIR / "local_welfare_detail_sample.xml"
